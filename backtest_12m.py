...
def load_one(symbol, file_urls):
    ...
    if not need.issubset(df.columns):
        df=df.reset_index(); df.columns=[str(x).lower() for x in df.columns]
        ren={}
        for c in df.columns:
            if c in ("timestamp","datetime","date_time","date","time","ts"): ren[c]="ts"
            elif c in ("open","o"): ren[c]="o"
...
def backtest_symbol(symbol, df):
    ...
    # TEST: volume >= 5x 20-bar average volume
        ...
        if pos is not None:
            if not pos["active"]:
                if r.h>=pos["entry"]*(1+TSL_ACTIVATION):
                    pos["active"]=True; pos["peak"]=max(pos["entry"]*(1+TSL_ACTIVATION),float(r.h))
            else:
...
def main():
    ...
    if not candidates: raise RuntimeError("No trades/data loaded")
    cols=["symbol","entry_ts","entry","exit_ts","exit","reason"]
    raw=pd.DataFrame(candidates,columns=cols); raw.entry_ts=pd.to_datetime(raw.entry_ts); raw.exit_ts=pd.to_datetime(raw.exit_ts); raw=raw.sort_values(["entry_ts","symbol"]).reset_index(drop=True)
    ...
    if t.empty: raise RuntimeError("No accepted trades after ₹50 lakh / 25-slot capital constraint")
    t["pnl_pct"]=(t.exit/t.entry-1)*100; t["capital"]=POSITION_CAPITAL; t["pnl_rupees"]=POSITION_CAPITAL*(t.exit/t.entry-1); t["capital_after_position"]=POSITION_CAPITAL+t.pnl_rupees; t["month"]=t.entry_ts.dt.strftime("%Y-%m"); t.to_csv(f"{OUT}/trades.csv",index=False)
    ...
    t=t.sort_values(["exit_ts","symbol"]).reset_index(drop=True); equity=TOTAL_CAPITAL; rows=[]
    for r in t.itertuples(index=False): equity+=r.pnl_rupees; rows.append([r.exit_ts,r.symbol,equity])
    eq=pd.DataFrame(rows,columns=["ts","symbol","equity"]); eq.to_csv(f"{OUT}/equity_curve.csv",index=False); final=float(eq.iloc[-1].equity)
    summary=pd.DataFrame([{"starting_capital":TOTAL_CAPITAL,"position_capital":POSITION_CAPITAL,"max_concurrent_positions":MAX_POSITIONS,"final_equity":final,"net_profit":final-TOTAL_CAPITAL,"return_pct":(final/TOTAL_CAPITAL-1)*100,"trades":len(t),"winners":int((t.pnl_pct>0).sum()),"losers":int((t.pnl_pct<=0).sum()),"win_pct":(t.pnl_pct>0).mean()*100,"avg_trade_pct":t.pnl_pct.mean(),"tsl_exits":int((t.reason=="TSL").sum()),"eod_exits":0,"strategy":"EMA200 + volume >= 5x 20-bar volume MA + close > prior 20-bar high; ₹50L total capital split into 25 x ₹2L positions; 1% activation; 1% TSL; no EOD exit"}]); summary.to_csv(f"{OUT}/summary.csv",index=False); print("\nMONTHLY RESULT\n",m.to_string(index=False),"\n\nSUMMARY\n",summary.to_string(index=False))
