...
def main():
    file_urls={year:get_repo_files(repo) for year,repo in REPOS}; syms=sorted(set(file_urls["2025"])|set(file_urls["2026"])); print(f"Found {len(syms)} parquet symbols")
    candidates=[]
    ...
    cols=["symbol","entry_ts","entry","exit_ts","exit","reason"]; raw=pd.DataFrame(candidates,columns=cols); raw.entry_ts=pd.to_datetime(raw.entry_ts); raw.exit_ts=pd.to_datetime(raw.exit_ts); raw=raw.sort_values(["entry_ts","symbol"]).reset_index(drop=True)
    accepted=[]; capital_free_at=START
    for r in raw.itertuples(index=False):
        if r.entry_ts>=capital_free_at: accepted.append(r); capital_free_at=r.exit_ts
